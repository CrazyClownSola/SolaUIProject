...
public class WoWAntiPathAnimation extends PageAnimation {
    ...
    public void play(View onView, float positionOffset) {
        ...
        if (positionOffset <= getStartOffset()) {
            ...
            if (onView instanceof WoWPathView) {
                ((WoWPathView) onView).setPercentage(0);
            }
            lastTimeIsLess = true;
            return;
        }
        ...
        if (positionOffset >= getEndOffset()) {
            ...
            if (onView instanceof WoWPathView) {
                ((WoWPathView) onView).setPercentage(1);
            }
        ...
        if (onView instanceof WoWPathView) {
            ((WoWPathView) onView).setPercentage(movementOffset);
        }
    }
}
